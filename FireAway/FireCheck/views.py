...
@login_required()
def device_list(request, pid):
    b = panel.objects.get(pk=pid)
    context= {
        "devices" : device.objects.all().filter(panel=b)
    }
...
@login_required()
def panel_list(request, bid):
    b = building.objects.get(pk=bid)
    context= {
        "panels" : panel.objects.all().filter(building=b)
    }
